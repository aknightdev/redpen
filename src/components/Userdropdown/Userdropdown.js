...
class Userdropdown extends React.Component {
    ...
    render() {
        ...
        if (isLoggedIn) {
          button= <div><div className="user_nm" onClick={this.gearClick}><button>{this.authUser.name.substring(0,2)}</button></div>
        ...
        if(this.state.showDrop)
            ...
            return <div className="user_nm" onClick={this.gearClick}><button>{this.authUser.name.substring(0,2)}</button></div>;
	}				
}
